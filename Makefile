...
COMMON_REPO ?= $(shell bash -c 'export MK_PATH=$(MK_PATH); echo $${MK_PATH%host/hbm_bandwidth/*}')
...
LDCLFLAGS_krnl_vaddmul += -DVDATA_SIZE=$(BUS_WIDTH) -DSTRIDE=$(STRIDE)
...
$(TEMP_DIR)/krnl_vaddmul.xo: src/krnl_vaddmul.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) -c -k krnl_vaddmul $(CLFLAGS) --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
$(BUILD_DIR)/krnl_vaddmul.xclbin: $(BINARY_CONTAINER_krnl_vaddmul_OBJS)
	mkdir -p $(BUILD_DIR)
ifeq ($(HOST_ARCH), x86)
	$(VPP) -l $(LDCLFLAGS) $(CLFLAGS) --temp_dir $(BUILD_DIR)  $(LDCLFLAGS_krnl_vaddmul) -o'$(BUILD_DIR)/krnl_vaddmul.link.xclbin' $(+)
	$(VPP) -p $(BUILD_DIR)/krnl_vaddmul.link.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/krnl_vaddmul.xclbin
else
	$(VPP) -l $(LDCLFLAGS) $(CLFLAGS) --temp_dir $(BUILD_DIR) -o'$(BUILD_DIR)/krnl_vaddmul.xclbin' $(+)
endif
...
ifneq ($(HOST_ARCH), x86)
	$(VPP) -p $(BUILD_DIR)/krnl_vaddmul.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) -o krnl_vaddmul.xclbin
endif
